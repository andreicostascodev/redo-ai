...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, ...props }, ref) => (
    <textarea
      className={cn(
        "flex w-full rounded-md border-none bg-transparent px-3 py-2.5 text-base text-gray-100 placeholder:text-gray-500 focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 min-h-[44px] resize-none scrollbar-thin",
        className
      )}
      ref={ref}
      rows={1}
      ...
      <TooltipProvider>
        ...
        >
          <div
            ref={ref}
            className={cn(
              "rounded-2xl border border-[#2a2a2f] bg-[#141417] p-3 shadow-[0_8px_30px_rgba(0,0,0,0.24)] transition-all duration-300",
...
> = ({ className, onKeyDown, disableAutosize = false, placeholder, ...props }) => {
  ...
  React.useEffect(() => {
    ...
    const max = typeof maxHeight === "number" ? maxHeight : 240;
...
const CustomDivider: React.FC = () => (
  <div className="relative h-6 w-[1.5px] mx-1">
    <div
      className="absolute inset-0 bg-gradient-to-t from-transparent via-orange-500/50 to-transparent rounded-full"
      style={{
        clipPath:
